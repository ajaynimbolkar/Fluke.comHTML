...
config. BalloonImgPath		= "/fluke_www/images/Web2_0/tooltip" // Path to images (border, corners, stem), in quotes. Path must be relative to your HTML file.
...
balloon.OnLoadConfig = function()
{
	if(tt_aV[BALLOON])
	{
		...
		if(tt_aV[BALLOONIMGPATH].charAt(tt_aV[BALLOONIMGPATH].length - 1) != 'http://www.fluke.com/')
			tt_aV[BALLOONIMGPATH] += "http://www.fluke.com/";
		return true;
	}
	return false;
};
...
{
	// Append slash to img path if missing
	if(config.BalloonImgPath.charAt(config.BalloonImgPath.length - 1) != 'http://www.fluke.com/')
		config.BalloonImgPath += "http://www.fluke.com/";
