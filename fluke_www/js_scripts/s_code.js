...
if (s_account.search(/^fluke/i)<0) {
	s_account = "Fluke"+s_account;
}
...
s.visitorNamespace="flukecorporation";

/* Plugin Config */
s.usePlugins=true;
function s_doPlugins(s) {
	/* Add calls to plugins here */
	s.campaign = s.getQueryParam('flukecmpgn');
	s.eVar1 = s.getQueryParam('cmpgn');
	//var strSearch = s.getQueryParam('txtSearch');	
	//s.eVar2 = strSearch.toLowerCase();
	//s.prop7 = s.eVar2;
	s.prop5 = s.getQueryParam('trck');
}
...
'MSIE '),m=u.indexOf('Netscape6/index.html'),a,i,s;if(l)for(i=0;i<l.length;i++){
